...
func ConfigError(baseURL string, options Options) string {
...
func ResolvedError(ctx context.Context, baseURL string, options Options, resolver IPResolver) string {
	if syncError := ConfigError(baseURL, options); syncError != "" {
		return syncError
	}
...
// ClassifyIP exposes the address classifier to callers that fetch
...
// ClassifyHost classifies a bare hostname the way assessDestination
