...
| `codexNativeModelContextModes?` | `{ "gpt-5.6-sol"?, "gpt-5.6-terra"?, "gpt-5.6-luna"?: "default" \| "1m" }` | Canonical Codex-login `openai` only. Each of the three GPT-5.6 models is opted into Codex's official 1M mode independently. A missing entry or `default` keeps that model's current 272k behavior plus the optional 922k context-cap workflow. For an opted-in model, sync writes `context_window = 1000000`, `max_context_window = 1000000`, and `auto_compact_token_limit = 900000` into that model's catalog row. No global root context override is written, so direct model switches inside Codex keep Default and 1M rows independent. Routed providers, GPT-5.5, GPT-5.4, Daybreak aliases, and `effective_context_window_percent` are unchanged. |
