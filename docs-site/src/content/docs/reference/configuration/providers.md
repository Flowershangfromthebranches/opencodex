...
| `codexNativeModelContextModes?` | `{ "gpt-5.6-sol"?, "gpt-5.6-terra"?, "gpt-5.6-luna"?: "default" \| "1m" }` | Canonical Codex-login `openai` only. Each of the three GPT-5.6 models is opted into Codex's official 1M mode independently (a missing entry or `default` keeps that model's current 272k behavior plus the optional 922k context-cap workflow). When an opted-in model is Codex's active model, sync writes `model_context_window = 1000000` and `model_auto_compact_token_limit = 900000` and raises only the opted-in rows' catalog `max_context_window` to 1,000,000; the root block is stripped again whenever any other model is active. Codex ≥0.147 clamps the root override against each model's catalog `max_context_window`, so other models never inherit 1M. Routed providers, GPT-5.5, GPT-5.4, Daybreak aliases, and `effective_context_window_percent` are unchanged. |
