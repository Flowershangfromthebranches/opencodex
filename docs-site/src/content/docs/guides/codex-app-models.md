...
| --- | --- |
| Codex login (account-qualified rows disabled) | Bare native ids such as `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; Pool or Direct is selected through `codexAccountMode`. Default mode keeps the 272,000-token catalog window. The Models card's per-model Default / 1M switch treats Sol, Terra, and Luna independently: each opted-in model gets a 1,000,000-token `context_window` and `max_context_window` plus a 900,000-token auto-compact limit in its catalog row. OpenCodex writes no global root context override, so direct model switches inside Codex preserve every model's selected mode. The pre-existing context cap can still opt into or cap the measured 922,000 workflow. |
