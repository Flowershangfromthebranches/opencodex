...
| --- | --- |
| Codex login (account-qualified rows disabled) | Bare native ids such as `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; Pool or Direct is selected through `codexAccountMode`. Default mode keeps the 272,000-token catalog window. The Models card's per-model Default / 1M switch treats Sol, Terra, and Luna independently: for each opted-in model the catalog default is preserved, only that row's `max_context_window` rises to 1,000,000, and Codex's 1,000,000 / 900,000 root opt-in is written while that model is Codex's active model (stripped again for any other active model). The pre-existing context cap can still opt into or cap the measured 922,000 workflow. |
