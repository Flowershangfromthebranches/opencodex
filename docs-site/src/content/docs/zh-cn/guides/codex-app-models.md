...
| 路由 | 选择器 id 与目录元数据 |
| --- | --- |
| Codex 登录（账户限定的选择器行未启用） | 显示 `gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna` 等裸原生 id，并按 `codexAccountMode` 使用 Pool 或 Direct。默认模式保留 272,000-token 目录窗口。Models 卡片中的 per-model Default / 1M 开关对 Sol、Terra、Luna 三者相互独立：每个开启 1M 的模型会在自己的目录行获得 1,000,000-token `context_window`、1,000,000-token `max_context_window` 和 900,000-token 自动压缩阈值。OpenCodex 不写入全局 root 上下文覆盖，因此在 Codex 内直接切换模型仍会保留每个模型各自的模式。原有上下文 cap 仍可启用或限制实测的 922,000 流程。 |
