...
| 路由 | 选择器 id 与目录元数据 |
| --- | --- |
| Codex 登录（账户限定的选择器行未启用） | 显示 `gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna` 等裸原生 id，并按 `codexAccountMode` 使用 Pool 或 Direct。默认模式保留 272,000-token 目录窗口。Models 卡片中的 per-model Default / 1M 开关对 Sol、Terra、Luna 三者相互独立：对每个开启 1M 的模型，目录默认值不变，只把该行的 `max_context_window` 提升为 1,000,000，并且仅当该模型是 Codex 当前活动模型时写入 Codex 的 1,000,000 / 900,000 root opt-in（活动模型切换为其他模型后该块会被移除）。原有上下文 cap 仍可启用或限制实测的 922,000 流程。 |
